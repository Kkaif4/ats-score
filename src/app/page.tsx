...
export default function Home() {
    ...
    return (
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 py-12 md:py-20 flex flex-col gap-12 justify-center items-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm text-gray-400">Loading ATS Resume Scorer...</span>
        </div>
      </div>
    );
  }
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent) => {
    ...
    if (!feedbackName || !feedbackEmail || !feedbackMessage) {
      setFeedbackStatus({ success: false, message: "Please fill out all fields." });
      return;
    }
    if (!feedbackCaptchaToken) {
      setFeedbackStatus({ success: false, message: "Please complete the CAPTCHA verification." });
      return;
    }
    ...
    try {
      ...
      setFeedbackStatus({ success: true, message: "Feedback submitted successfully! Thank you." });
      ...
      setFeedbackStatus({ success: false, message: err.message || "Failed to submit feedback." });
      setFeedbackCaptchaToken("");
      if (feedbackTurnstileRef.current) {
        feedbackTurnstileRef.current.reset();
      }
    ...
    <div className="flex-1 w-full max-w-7xl mx-auto px-4 py-8 sm:py-12 md:py-20 flex flex-col gap-8 md:gap-12">
      ...
      <header className="text-center flex flex-col items-center gap-3 md:gap-4">
        ...
        <p className="text-gray-400 max-w-xl text-sm sm:text-base md:text-lg px-2">
          Evaluate resume performance against industry-standard parser patterns. Upload PDF or DOCX format to receive matching metrics.
        </p>
      </header>
      ...
      <main className="w-full">
        ...
        <section className="w-full max-w-4xl mx-auto flex flex-col gap-6 md:gap-8">
          <div className="glass-panel rounded-2xl p-4 sm:p-6 md:p-8">
            ...
            <form onSubmit={handleAnalyzeResume} className="flex flex-col gap-5 md:gap-6">
              {/* Drag and drop zone */}
              <div
              ...
              >
                ...
                {file ? (
                  <div className="flex flex-col items-center">
                    <p className="text-sm font-semibold text-blue-400 break-all px-2">{file.name}</p>
                    <p className="text-xs text-gray-500">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                ) : (
                  <div>
                    <p className="text-sm font-medium px-2">Drag & drop files here or click to browse</p>
                    <p className="text-xs text-gray-500 mt-1">Supports PDF or DOCX format up to 5MB</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <label htmlFor="jd-input" className="text-sm font-semibold text-gray-300 flex items-center gap-1.5">
                    <svg className="w-4 h-4 text-blue-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    Job Description
                  </label>
                  <span className="text-[10px] sm:text-xs text-gray-500 font-medium bg-gray-800/40 px-2 py-0.5 rounded border border-gray-800">Optional</span>
                </div>
                <textarea
                  id="jd-input"
                  rows={4}
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-gray-900/20 p-3 sm:p-4 rounded-xl border border-gray-800 w-full overflow-hidden">
                ...
                <button
                  type="submit"
                  disabled={!isMounted || isProcessing || !file || !docCaptchaToken}
                ...
                >
                  {isProcessing ? (
                    <span className="flex items-center justify-center gap-2">
                      <svg className="animate-spin h-5 w-5 text-white animate-spin-slow" fill="none" viewBox="0 0 24 24">
            ...
            <div className="glass-panel rounded-2xl p-4 sm:p-6 md:p-8 animate-fade-in glow-secondary">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 border-b border-gray-800 pb-4 md:pb-6 mb-4 md:mb-6">
                ...
                <div className="flex-1 text-center md:text-left">
                  <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">Evaluation Summary</h3>
                  ...
                  {analysisResult.matchingKeywords.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {analysisResult.matchingKeywords.map((tag, idx) => (
                        <span key={idx} className="bg-green-500/10 border border-green-500/20 text-green-400 text-[10px] sm:text-xs px-2.5 py-1 rounded-md font-medium break-words max-w-full">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  {analysisResult.missingKeywords.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {analysisResult.missingKeywords.map((tag, idx) => (
                        <span key={idx} className="bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] sm:text-xs px-2.5 py-1 rounded-md font-medium break-words max-w-full">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500 italic">No critical missing keywords identified</p>
                  )}
                </div>
                  ...
                  {analysisResult.recommendations.length > 0 ? (
                    <ul className="space-y-2 sm:space-y-2.5">
                      {analysisResult.recommendations.map((step, idx) => (
                        <li key={idx} className="text-xs sm:text-sm text-gray-300 flex items-start gap-2 sm:gap-2.5">
                    ...
                    <p className="text-xs text-gray-500 italic">Resume is perfectly formatted and optimization-ready</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-fade-in">
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 md:p-8 relative animate-scale-up glow-secondary border border-gray-800 bg-gray-950/95">
            ...
            <p className="text-xs text-gray-500 mb-6">
              Your systemic telemetry (IP, OS, browser) is recorded securely alongside your submission to mitigate spam and abuse.
            </p>
            ...
            <form onSubmit={handleFeedbackSubmit} className="flex flex-col gap-5">
              ...
              <button
                type="submit"
                disabled={!isMounted || isSubmittingFeedback || !feedbackCaptchaToken}
      ...
      {isProcessing && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="flex flex-col items-center gap-6 max-w-sm text-center p-6 animate-scale-up">
            ...
            <div className="flex flex-col gap-2">
              <h3 className="text-xl font-bold text-white tracking-tight">Analyzing Resume</h3>
              <p className="text-sm text-gray-400">
                Gemini is extracting resume content, checking keywords, matching with job requirements, and calculating score...
              </p>
            </div>
